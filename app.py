...
if submit:
    if not api_key:
        ...
        try:
            ...
            # 3. Gemini 설정 및 업로드 부분
            video_file = genai.upload_file(path=video_path)
            st.write(f"업로드된 파일 정보: {video_file.display_name} ({os.path.getsize(video_path) / 1024 / 1024:.2f} MB)")
            ...
            if video_file.state.name != "ACTIVE":
                # FAILED일 경우 구체적인 이유가 있는지 확인 (지원되는 경우)
                st.error(f"Gemini 영상 처리 실패. 상태: {video_file.state.name}")
                # 실패 원인을 찾기 위해 파일 삭제 후 중단
